...
class _ExamAttemptPageState extends State<ExamAttemptPage> {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () {
              debugPrint("تم إلغاء التسليم، الطالب عاد للامتحان");
          ...
          ElevatedButton(
            ...
            onPressed: () {
              debugPrint("تم تأكيد التسليم، يتم الآن إرسال الإجابات للسيرفر");
  ...
  void _executeSubmit() {
    ...
    final Map<String, dynamic> submitPayload = {
      ...
      "answers": _selectedAnswers.map(
        (qId, optId) => MapEntry(qId.toString(), optId),
      ),
    };

    //  تفعيل الاستدعاء بشكل رسمي الآن
    context.read<ExamsCubit>().submitExamAnswers(submitPayload);
    debugPrint("تم تجهيز بيانات التسليم وإرسالها للـ Cubit: $submitPayload");
  }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop:
          false, // منع السحب أو العودة بالخلف تماماً لضمان عدم ضياع الإجابات
      child: AppScaffold(
          ...
          children: [
            ...
            Expanded(
              child: _questions.isEmpty
                  ? const Center(
                      child: CustomText('لا توجد أسئلة في هذا الامتحان'),
                    )
                  : PageView.builder(
          ...
          children: [
            ...
            ...List.generate(question.options.length, (index) {
              ...
              return GestureDetector(
                ...
                onTap: () =>
                    setState(() => _selectedAnswers[question.id] = option.id),
                child: Container(
